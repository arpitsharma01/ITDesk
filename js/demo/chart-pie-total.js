...
function fetchtotalCategory() {
  ...
  var dataArray=[];
        fetch(`https://localhost:44327/api/deviceInfo/categoryList`, {
          ...
          .then((res) => res.json())
          .then((data) => {
            var colors = ['#ec5800', '#ffb62e', '#ffc861', '#f09c00', '#b87700'];
          var i=0;
          let l=``;
          
            data.forEach((TempUser) => {
              labelArray.push(TempUser.deviceType);
              dataArray.push(TempUser.totalCount);
            ...
            document.getElementById("te-to").innerHTML= l;
            var ctx = document.getElementById("totalpie");
